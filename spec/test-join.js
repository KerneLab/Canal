...
describe("Test join", function()
{
	...
	it("join() partially", function()
	{
		var result = Canal.of([ [ "a", 1 ], [ "b", 2 ], [ "b", 3 ], [ "c", 4 ] ]).join(
				Canal.of([ [ "b", 1 ], [ "c", 2 ], [ "d", 3 ] ])).take(2);
		expect(result).to.eql([ [ "b", [ 2, 1 ] ], [ "b", [ 3, 1 ] ] ]);
	});

	it("join() filter", function()
	{
		var result = Canal.of([ [ "a", 1 ], [ "b", 2 ], [ "b", 3 ], [ "c", 4 ] ]).join(
				Canal.of([ [ "b", 1 ], [ "c", 2 ], [ "d", 3 ] ])).filter(function(data)
		{
			return data[0] == "c";
		}).collect();
		expect(result).to.eql([ [ "c", [ 4, 2 ] ] ]);
	});
});
